...
Int TSKMESSAGE_execute(TSKMESSAGE_TransferInfo* info)
{
    ...
    for (i = 0; (((info->numTransfers == 0) || (i < info->numTransfers)) && (status == SYS_OK)); i++)
    {
		...
		for (i = 0;i < SIZE; i++)
		{
			for (j = 0; j < SIZE; j++)
			{
				*(msg->result + (i*SIZE + j)) = 0;
				//prod[i][j] = 0;
				for(k=0;k<SIZE;k++)
					//prod[i][j] += *(msg->inp1+(i*SIZE+k)) * *(msg->inp2+(k*SIZE+j));
					*(msg->result + (i*SIZE + j)) +=  *(msg->inp1+(i*SIZE+k)) * *(msg->inp2+(k*SIZE+j));
			}
		}
		//msg->result = &prod[0][0];
		msg->check[0] = *(msg->result);
		msg->check[1] = *(msg->result + 1);
		msg->check[2] = *(msg->result + 2);
		msg->check[3] = *(msg->result + 3);		
