...
#define SAMPLEMQT_CTRLMSG_SIZE  ZCPYMQT_CTRLMSG_SIZE
    ...
    Uint16  mat1[SIZE][SIZE], mat2[SIZE][SIZE], prod[SIZE][SIZE];    
    //void generateMatrix();

    void generateMatrix(void)
    {
	//SYSTEM_0Print("Entered generateMatrix()\n");
	Uint16 i;
	Uint16 j;
	SYSTEM_0Print("Entered generateMatrix()\n");	
	...
	SYSTEM_0Print("Exiting generateMatrix()\n");
	...
	//Start generating matrices for multiplication
	generateMatrix();
        ...
        for (i = 1 ; ((numIterations == 0) || (i <= (numIterations + 1))) && (DSP_SUCCEEDED (status)); i++)
        {
            ...
            {
		SYSTEM_1Print("Message received: %s\n", (Uint32) msg->arg1);
		//SYSTEM_1Print("Value Received: %ld\n",  msg->arg2[0]);
	    }
            ...
            {
                //MSGQ_free((MsgqMsg) msg);
		SYSTEM_0Print("Multiplication Done:\n");
		for (k = 0;k < SIZE; ++k)
		{
			SYSTEM_0Print("\n");
			for (j = 0; j < SIZE; ++j)
			{
				//prod[k][j] = *(msg->result + k*SIZE + j);
				SYSTEM_1Print("\t%d ", prod[k][j]);
			}
		}
		for(k=0;k<4;++k)		
			SYSTEM_1Print("%d ", msg->check[k]);
		MSGQ_free((MsgqMsg) msg);
		SYSTEM_0Print("\n");
            }
            else
