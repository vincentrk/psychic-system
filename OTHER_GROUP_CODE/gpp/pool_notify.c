...
#define pool_notify_IPS_EVENTNO           5
            ...
            IN Uint8   processorId)
    {
        ...
        if (DSP_SUCCEEDED (status)) {
            ...
            if (DSP_FAILED (status)) {
                printf ("PROC_load (%s) failed. Status = [0x%x]\n", dspExecutable, (int)status) ;
                return -1;
            }
        }
     ...
     *
     *  @desc   This function gets the shared memory pointer
     *
     *  ============================================================================
     */
    NORMAL_API char *const pool_notify_GetSharedMem(void)
    {
        return pool_notify_DataBuf;
    }

    /** ============================================================================
     *  @func   pool_notify_NotifySharedMem
     *
     *  @desc   This function notifies the DSP that it can use the shared memory
     *
     *  ============================================================================
     */
    NORMAL_API void pool_notify_NotifySharedMem(long int size)
    {
        /* Write back the shared memory */
        if (size == -1) {
            POOL_writeback (POOL_makePoolId(0, SAMPLE_POOL_ID),
                    pool_notify_DataBuf,
                    pool_notify_BufferSize);
        } else {
            POOL_writeback (POOL_makePoolId(0, SAMPLE_POOL_ID),
        ...
        /* Notify the DSP with the shared memory */
        NOTIFY_notify (0,pool_notify_IPS_ID,pool_notify_IPS_EVENTNO,0);
    }

    /** ============================================================================
     *  @func   pool_notify_WaitDSP
     *
     *  @desc   This function waits for the DSP to be done using the shared memory
     *
     *  ============================================================================
     */
     ...
     *
     *  @desc   This function notifies the DSP that it can stop execution
     *
     *  ============================================================================
     */
    NORMAL_API void pool_notify_Done(void)
    {
        NOTIFY_notify (0,pool_notify_IPS_ID,pool_notify_IPS_EVENTNO,1);
    }
     ...
     *  ============================================================================
     */
    NORMAL_API DSP_STATUS pool_notify_Init (IN Char8 * dspExecutable, IN Char8 * strBufferSize)
    {
        DSP_STATUS status       = DSP_SOK ;
        Uint8      processorId  = 0 ;
